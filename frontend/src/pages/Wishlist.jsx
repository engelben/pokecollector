...
function WishlistActions({ item, onEdit, onRemove, onAddToCollection, t, compact = false }) {
  return (
    <div className={`${compact ? 'flex justify-end gap-1.5' : 'mt-3 flex flex-wrap gap-2'}`} data-cart-action-area="wishlist-card">
      <button type="button" className={`btn-ghost text-blue hover:bg-blue/10 ${compact ? 'min-h-10 min-w-10 p-2' : 'min-h-10 py-1.5'}`} onClick={onEdit} aria-label={t('wishlist.editCard')}><Edit2 size={19} /> {!compact && t('common.edit')}</button>
      {item.cardmarket_url && <a className={`btn-ghost text-purple-400 hover:bg-purple-500/10 ${compact ? 'min-h-10 min-w-10 p-2' : 'min-h-10 py-1.5'}`} href={item.cardmarket_url} target="_blank" rel="noreferrer" aria-label="Cardmarket"><ExternalLink size={19} /> {!compact && 'Cardmarket'}</a>}
      <button type="button" className={`btn-ghost text-green hover:bg-green/10 ${compact ? 'min-h-10 min-w-10 p-2' : 'min-h-10 py-1.5'}`} onClick={onAddToCollection} aria-label={t('wishlist.addToCollection')}><Plus size={20} /> {!compact && t('wishlist.addToCollection')}</button>
      <button type="button" className={`btn-ghost min-h-10 text-brand-red hover:bg-brand-red/10 ${compact ? 'min-w-10 p-2' : 'py-1.5'}`} onClick={onRemove} aria-label={t('wishlist.removeCard')}><Trash2 size={19} /> <span className="sr-only">{t('wishlist.removeCard')}</span></button>
    </div>
  )
}
                    ...
                    <div className="grid items-center gap-2 px-3 py-2 lg:grid-cols-[minmax(190px,2fr)_minmax(120px,1fr)_110px_100px_112px_100px_110px_184px] lg:gap-3">
                      <div className="flex min-w-0 items-center gap-2">
                        <div className="group relative h-14 w-10 shrink-0 overflow-visible rounded bg-bg-elevated">
                          ...
                          <div className="pointer-events-none absolute left-full top-1/2 z-50 ml-3 hidden w-52 -translate-y-1/2 overflow-hidden rounded-xl border border-border bg-bg-surface p-1 shadow-2xl group-hover:block">
                      ...
                      <p className="text-sm font-bold text-gold">{price == null ? '—' : formatPrice(price)}</p>
